...
update.theta <- function(SigmaInv.diag, VInv.diag, u, v, sv, w) {
    ...
    VinvM   <- F.prime %*% (SigmaInv.diag * (alpha[1,] - (u[which.countryTime] + v[which.regionTime] + sv[which.sregionTime] + w[which.time]) - varphiPlusC.age))
...
tracePlots <- function() {
    pdf(paste0(outdirname, filename, "_univariate_traceplots",".pdf"), width=10, height=7.5); par(mfrow=c(3,2), mar=c(0.5,4,0.5,0))  # Opens PDF document
        ...
        for(i in 1:(2*ng))  plot(gamma[,i], type='l', xlab='', ylab=paste('gamma', i),xaxt='n')           # Age model parameters: global random intercept and slope (log psi and log phi)
        for(i in 1:ng) { plot(log(sigma1_c[,i]), type='l', xlab='', ylab=paste('sigma1_c', i),xaxt='n') } # Log variances for country-specific random intercept in spline coefficients (log c)
        for(i in 1:ng) { plot(log(sigma2_c[,i]), type='l', xlab='', ylab=paste('sigma2_c', i),xaxt='n') } # Log variances for country-specific random slope in spline coefficients (log omiga_c)
        for(i in 1:ng) { plot(log(sigma1_r[,i]), type='l', xlab='', ylab=paste('sigma1_r', i),xaxt='n') } # Log variances for region-specific random intercept in spline coefficients (log r)
        for(i in 1:ng) { plot(log(sigma2_r[,i]), type='l', xlab='', ylab=paste('sigma2_r', i),xaxt='n') } # Log variances for region-specific random slope in spline coefficients (log omiga_r)
        for(i in 1:ng) { plot(log(sigma1_s[,i]), type='l', xlab='', ylab=paste('sigma1_s', i),xaxt='n') } # Log variances for superregion-specific random intercept in spline coefficients (log s)
        for(i in 1:ng) { plot(log(sigma2_s[,i]), type='l', xlab='', ylab=paste('sigma2_s', i),xaxt='n') } # Log variances for superregion-specific random slope in spline coefficients (log omiga_s)
